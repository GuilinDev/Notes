...
1) 从root开始做一个中序遍历，返回一个含有所有结点的array（儿子为空依然要加入到array中），最后判断Arrays.equals(arr1, arr2)是否相等，不太好；2）递归，递归的实现方式有多种，这里推荐一种思路比较清晰的：设定一个min，代表右子树里面的最小值，设定一个max代表左子树里面的最大值，每次递归到当前结点为root时，判断当前结点是否在min和max之间。
